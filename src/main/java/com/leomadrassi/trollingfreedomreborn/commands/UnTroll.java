...
public class UnTroll implements CommandExecutor, TabCompleter {
    ...
    public void stopSpecificTroll(Player victim, String trollName, Player sndr) {
        ...
        switch (name) {
            // --- Page 1 ---
            case "annoy":
                Annoy.stopAnnoy(victim);
                break; // Repeating task handled in Step 1
            case "anvildrop":
                break; // Repeating task handled in Step 1
            case "aquaphobia":
                new Aquaphobia().unAqua(victim);
                break;
            case "bedexplosion":
                new BedExplosion().UnBedExplosion(victim);
                break;
            case "stopblockbreakplace":
                new Break().unBreak(victim);
                break;
            case "cage":
                new Cage().UnCage(victim);
                break;
            case "cavesounds":
                break; // Repeating task handled in Step 1
            case "randomchat":
                new ChatChange().UnChatChange(victim);
                break;
            case "coffindance":
                new Coffin().chk(new org.bukkit.entity.ArmorStand[0]);
                break;
            case "entitymultiply":
                new EntityMultiply().UnEntityMultiply(victim);
                break;
            case "deafen":
                Deafen.UnDeafen(victim);
                break;
            case "randomcraft":
                new RandomCrafts().unCraftTroll(victim);
                break;

            // --- Page 2 ---
            case "forcejump":
                new ForceJump().UnJump(victim);
                break;
            case "freeze":
                new Freeze().Unfreeze(victim);
                break;
            case "hideallplayers":
                new HideAllPlayers().UnHideAll(victim);
                break;
            case "instatoolbreak":
                new InstaToolBreak().UnInstaToolBreak(victim);
                break;
            case "inventorystop":
                new InventoryStop().UnInventoryStop(victim);
                break;
            case "kittycannon":
                new KittyCannon().UnKittyCannon(victim);
                break;
            case "lag":
                break; // Repeating task handled in Step 1
            case "lightning":
                new Lightning().UnLightning(victim);
                break;
            case "lockinventory":
                break;
            case "nick":
                new NickWithoutEss().UnNick(victim);
                break;
            ...
            case "burn":
                ...
                break;
            case "potato":
                new Potato().unpotato(victim);
                break;
            case "pumpkin":
                new Pumpkin().unPumpkin(victim);
                break;
            case "rainitems":
                new RainItems().UnRainItem(victim);
                break;
            case "randominv":
                new RandomInv().UnRandomInv(victim);
                break;
            case "randomparticle":
                new RandomParticle().UnRandomParticle(victim);
                break;
            case "rickroll":
                new RickRoll().UnRickRoll(victim);
                break;
            case "silverfish":
                new Silverfish().UnFish(victim);
                break;
            case "slenderman":
                new Slenderman().UnEnderman(victim);
                break;
            case "slipperyhands":
                break; // Repeating task handled in Step 1
            case "sneakdestroy":
                new SneakDestroy().UnSneakDestroy(victim);
                break;
            case "spin":
                new Spin().StopSpin(victim);
                break;
            case "starve":
                new Starve().UnStarve(victim);
                break;
            case "skyflash":
                new TimeFlash().UnSkyFlash(victim);
                break;
            ...
            case "ghastsound":
                break; // Repeating task handled in Step 1
            case "tntplace":
                new TNTPlace().UnTNTPlace(victim);
                break;
            case "vomit":
                new Vomit().UnVomit(victim);
                break;
            case "explodeonchat":
                ExplodeOnChat.UnChat(victim);
                break;
            case "inventoryrave":
                try {
                    new InventoryRave().UnInvRave(victim);
                } catch (Exception ignored) {
                }
                break;
            case "invert":
                InvertWalk.UnInvert(victim);
                break;
            case "stopsleep":
                new BedNight().UnStopSleep(victim);
                break;
            case "reversemessage":
                new ReverseMessage().UnReverse(victim);
                break;
            case "poop":
                new Poop().UnPoop(victim);
                break;
            case "control":
                if (sndr != null) {
                    sndr.performCommand("control stop");
